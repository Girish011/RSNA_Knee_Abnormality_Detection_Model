# Kaggle GPU T4 — DINOv2-B fold0, frozen, v6c labels + GCE loss (patch shadows code loss.py).
import json
import os
import subprocess
import sys
...
PATCH = Path("/kaggle/input/datasets/girishbose/rsna-knee-loss-gce")
...
cfg = PATCH / "configs" / "v6c_gce.yaml"
if not cfg.exists():
    cfg = _find("v6c_gce.yaml")
folds = CODE / "data/folds/folds_v1.csv"
out = Path(f"/kaggle/working/main_dinov2_b_v6c_gce/fold{FOLD}")
...
if hist.exists():
    ...
    print(f"BEST fold{FOLD} val_macro_auc (v6c_gce) = {best:.4f}", flush=True)
