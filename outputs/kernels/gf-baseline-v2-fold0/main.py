"""Train gf_baseline_v2 fold0 (frozen DINOv2-S, 12×336 cache) then score all 58 gold.

Writes under /kaggle/working/gf_baseline_v2/:
...
CACHE_CANDIDATES = [
    Path("/kaggle/input/datasets/girishbose/rsna-knee-cache-gf-v2/cache_gf_v2"),
    Path("/kaggle/input/rsna-knee-cache-gf-v2/cache_gf_v2"),
    Path("/kaggle/input/notebooks/girishbose/gf-cache-v2/cache_gf_v2"),
    Path("/kaggle/input/girishbose-gf-cache-v2/cache_gf_v2"),
    Path("/kaggle/input/gf-cache-v2/cache_gf_v2"),
]
CACHE = next((p for p in CACHE_CANDIDATES if p.exists()), None)
if CACHE is None:
    hits = list(Path("/kaggle/input").rglob("cache_gf_v2"))
    CACHE = hits[0] if hits else Path("/kaggle/input/MISSING_cache_gf_v2")

TRAIN_CSV = Path("/kaggle/input/competitions/rsna-knee-abnormality-detection/train.csv")
OUT = Path("/kaggle/working/gf_baseline_v2")
...
def main() -> None:
    ...
    print("=== gf_baseline_v2 fold0 vs full-58 gold ===")
