"""Train gf_labels_lig2 folds 0-4 on cache_gf_v0, then true OOF gold-58.

Teacher: weak_labels_gf_lig2.csv (weak_v1 + Medial Meniscus gap-fill only).
...
META = Path("/kaggle/input/datasets/girishbose/rsna-knee-gf-lig2-meta")
if not META.exists():
    META = Path("/kaggle/input/rsna-knee-gf-lig2-meta")
...
OUT = Path("/kaggle/working/gf_labels_lig2_5fold")
N_FOLDS = 5
...
def main() -> None:
    ...
    weak = META / "data" / "processed" / "weak_labels_gf_lig2.csv"
    ...
    for fold in range(N_FOLDS):
        cmd = [
            ...
            "--config",
            "configs/gf_labels_lig2.yaml",
            "--train-csv",
            str(TRAIN_CSV),
            "--folds",
            "data/folds/folds_v1.csv",
            "--weak-csv",
            "data/processed/weak_labels_gf_lig2.csv",
            "--cache-dir",
            str(CACHE),
            "--fold",
            str(fold),
    ...
    print("=== gf_labels_lig2 5-fold OOF vs weak(+expert) ===", flush=True)
    ...
    print("=== gf_labels_lig2 5-fold OOF vs full-58 gold ===", flush=True)
        ...
        json.dumps(
            {
                ...
                "note": "True OOF gold; teacher=gf_lig2 Med-Men-only gap-fill on weak_v1",
            },
            indent=2,
        )
    )
